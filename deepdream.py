...
inception5h.maybe_download() #Only download if file not present
model = inception5h.Inception5h()
len(model.layer_tensors)



def load_image(filename): #Opens image, returns as a numpy float
    image = PIL.Image.open(filename)

    return np.float32(image)

def save_image(image, filename): #Converts image from numpy array into jpeg then saves it
...
def plot_image(image): #Show user image progress
...
def resize_image(image, size=None, factor=None): #Resize if image too large
...
def optimize_image(layer_tensor, image,
    ...
    for i in range(num_iterations):
        ...
        if show_gradient:
            print("Gradient min: {0:>9.6f}, max: {1:>9.6f}, stepsize: {2:>9.2f}".format(grad.min(), grad.max(), step_size_scaled))
            # Plot the gradient.
            plot_gradient(grad)
        else:
...
while picNum <= picTotal:
    ...
    while layer_num < 12:
        ...
        image = load_image(filename='input/{}.jpg'.format(picNum))
        plot_image(image)

        layer_tensor = model.layer_tensors[layer_num]
        layer_tensor
